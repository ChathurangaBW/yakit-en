...
const httpHeaderSuggestions = [
  ...
  } as languages.CompletionItem,
  {
    ...
    label: 'User-Agent',
    insertText:
      'User-Agent: ${1:Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36}',
    insertTextRules: languages.CompletionItemInsertTextRule.InsertAsSnippet,
...
export const getHTTPHeaderSuggestions = async (position: monaco.Position) => {
  ...
  const transformCustomCode: any = targetCustomCode.map((it) => {
    return {
      documentation: {
        value: highlightKinds.includes(it.Level) ? '```\n' + it.Code + '\n```' : it.Code,
        isTrusted: true,
      },
      ...
      insertText: it.Code,
      insertTextRules: languages.CompletionItemInsertTextRule.InsertAsSnippet,
      sortText: getSortTextByKindAndLabel(it.Level, it.Name),
    }
  })
  const targetSuggestions = httpHeaderSuggestions?.concat(transformCustomCode)
...
export const getWordAtPositionWithSep = (
...
export const newFuzztagCompletionHandlerProvider = (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    iWord = getWordAtPositionWithSep(
      model,
      new monaco.Position(position.lineNumber, iWord.startColumn - parenthesesWord.length),
      '{{',
    )
    if (!iWord.word.includes('{{')) {
      const suggestions = await getHTTPHeaderSuggestions(position)
    ...
    iWord = {
      word: iWord.word + parenthesesWord,
...
monaco.languages.setMonarchTokensProvider(fuzzHTTPMonacoSpec, {
  ...
  tokenizer: {
    ...
    http_header: [
      ...
      [
        /(Content-Length|Host|Origin|Referer)(:)/g,
        ['http.header.danger', { token: 'delimiter', next: '@http_header_value' }],
      ],
      [
        /(Authorization|X-Forward|Real|User-Agent|Protection|CSP|X-Requested-With)(:)/g,
        ['http.header.danger', { token: 'delimiter', next: '@http_header_value' }],
      ],
