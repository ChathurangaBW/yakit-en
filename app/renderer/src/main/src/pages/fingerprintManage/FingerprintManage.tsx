...
const LocalFingerprintTable: React.FC<LocalFingerprintTableProps> = memo((props) => {
  ...
  const defaultColumns: (ColumnTypes[number] & {
    ...
    },
    {
      ...
      ellipsis: true,
      render: (text) => (
        <div className={classNames(styles['basic'], 'yakit-content-single-ellipsis')} style={{ overflow: 'hidden' }}>
          {text}
        </div>
      ),
    },
  ...
  const handleSave = (row: FingerprintRule, newRow: FingerprintRule) => {
    ...
    if (newRow.RuleName.length && newRow.MatchExpression.length) {
      ...
      if (row.RuleName !== newRow.RuleName || row.MatchExpression !== newRow.MatchExpression || groupNameChangeFlag) {
        grpcUpdateFingerprint({
          Id: row.Id,
          Rule: {
            ...row,
            ...
            if (editInfoRef.current) {
              grpcUpdateFingerprint({
                ...
                .then(() => {
                  yakitNotify('success', '更新成功')
                  if (editInfoRef.current) {
                    if (JSON.stringify(value.GroupName) !== JSON.stringify(editInfoRef.current.GroupName)) {
                      onSetRefreshLocalGroup((prev) => !prev)
                    }
                  }
                  editInfoRef.current = undefined
          ...
          <div className={styles['add-and-remove-group']}>
            ...
            <div className={styles['group-list']}>
              ...
              {showGroup.map((item) => {
                ...
                const isCheck = [...addGroup, ...removeGroup].findIndex((item) => item.GroupName === GroupName) > -1
                return (
                  <div
                    key={item.GroupName}
