...
export const WebShellURLTreeAndTable: React.FC<WebShellURLTreeAndTableProp> = (props) => {
      ...
      },
      {
        ...
        render: (_, t: TreeNode) => (
          <YakitTag color="bluePurple">
            {t.data?.Extra.filter((i) => i.Key === 'perm')
              .map((i) => i.Value)
              .join('')}
          </YakitTag>
        ),
  ...
  const [currentYakURL, setCurrentYakURL] = useState<string>(
    `${props.shellType}:///${props.CurrentPath}?op=file&mode=list&id=${props.Id}`,
  )
  ...
  const showFile = (url: YakURL) => {
    ...
    requestYakURLList({ url: newYakUrl })
      .then((rsp) => {
