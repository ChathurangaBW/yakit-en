...
const ConnectivityCheckAction: React.FC = React.memo(() => {
  ...
  return (
    <>
      <Tooltip
        title={
          isConnectivityChecking ? t('PluginTunHijack.connectivityChecking') : t('PluginTunHijack.connectivityCheck')
        }
      >
        <YakitButton
  ...
  (props: TunHijackProcessTableProps) => {
    ...
    const updateTableDate = useThrottleFn(
      (init?: boolean) => {
        ...
        if (searchVal) {
          newTableData = newTableData.filter((item) => item.Name.toLowerCase().includes(searchVal.toLowerCase()))
        }
