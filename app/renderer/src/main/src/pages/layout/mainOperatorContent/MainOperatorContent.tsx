...
const pageTabItemRightOperation: (t: TFunction) => YakitMenuItemType[] = (t) => {
  return [
    ...
    {
      label: t('MainOperatorContent.moveToGroup'),
      key: 'addToGroup',
      children: [
        // {
        //     label: "新建组",
    ...
    {
      label: t('MainOperatorContent.closeCurrentTab'),
      key: 'remove',
    },
    {
      label: t('MainOperatorContent.closeOtherTabs'),
  ...
  if (isMemfit()) {
    return [
      ...
      },
      {
        ...
        verbose: '知识库',
        verboseKey: 'YakitRoute.ai-repository',
...
export const MainOperatorContent: React.FC<MainOperatorContentProps> = React.memo((props) => {
  ...
  const onSwitchMenuItem = useMemoizedFn((data) => {
    try {
      ...
      yakitNotify('error', t('MainOperatorContent.switchMenuItemFailed', { error: `${error}` }))
    }
  })
  /**
   * 更新单例页面的标签名
  ...
  const addBugTest = useMemoizedFn((type: number, res?: any) => {
    ...
    if (type === 1 && URL) {
      ...
      apiFetchQueryYakScriptGroupLocal(false, [], 2)
        ...
        .catch((err) => yakitNotify('error', t('MainOperatorContent.fetchPluginGroupFailed', { error: `${err}` })))
    }
    if (type === 2) {
      openMenuPage(
    ...
    if (!isModalVisibleRef.current) {
      ...
      if (data) {
        const info: OnlyPageCache = {
          route: data.route,
          menuName:
            data.route === YakitRoute.Plugin_OP ? data.pluginName || '' : YakitRouteToPageInfo[data.route]?.label || '',
      ...
      if (SingletonPageRoute.includes(route)) {
        ...
        switch (route) {
          case YakitRoute.HTTPFuzzer:
            verbose = nodeParams?.verbose || `${filterPage.length > 0 ? (filterPage[0].multipleLength || 0) + 1 : 1}`
            break
          case YakitRoute.Modify_Notepad:
            verbose =
              nodeParams?.verbose || `Untitled-${filterPage.length > 0 ? (filterPage[0].multipleLength || 0) + 1 : 1}`
            break
          default:
            break
        }
  ...
  const getFuzzerSequenceCache = useMemoizedFn(() => {
    getRemoteProjectValue(FuzzerRemoteGV.FuzzerSequenceCache).then((res: any) => {
      try {
        ...
        yakitNotify('error', t('MainOperatorContent.webFuzzerParseFailed', { error: `${error}` }))
      }
    })
  })
  ...
  const fetchFuzzerList = useMemoizedFn(async (cache, add, openFlag = true) => {
    try {
      ...
      yakitNotify('error', t('MainOperatorContent.fetchFuzzerListFailed', { error: `${error}` }))
    }
  })
    ...
    return new Promise(async (resolve) => {
      apiQueryFuzzerConfig(query).then(async ({ Data = [] }) => {
        try {
          ...
          yakitNotify('error', t('MainOperatorContent.restoreHistoryFailed', { error: error + '' }))
        }
      })
    })
  })
            ...
            help={
              bugList.length === 0 && (
                <span className={styles['bug-test-help']}>
                  {t('MainOperatorContent.manageNewGroupHint')}
                  <span
    ...
    showByRightContext(
      {
        ...
        data: [
          ...
          {
            label: t('MainOperatorContent.closeAllTabs'),
            key: 'removeAll',
          },
          {
            label: t('MainOperatorContent.closeOtherTabs'),
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="droppable1" direction="horizontal">
        {(provided, snapshot) => {
          return (
            <div className={classNames(styles['tab-menu-first'])} {...provided.droppableProps} ref={provided.innerRef}>
  ...
  React.forwardRef((props, ref) => {
    ...
    const mergingGroup = useMemoizedFn((result: DropResult) => {
      ...
      if (sourceGroupChildrenLength > 0 && combineGroupChildrenLength > 0) {
        ...
        const groupId =
          sourceGroupChildrenLength > 0 || combineGroupChildrenLength > 0 ? subPage[combineIndex].id : generateGroupId()
    ...
    const onAddSubPage = useMemoizedFn(() => {
      if (getSubPageTotal(subPage) >= secondaryTabsNumRef.current) {
        yakitNotify('error', t('MainOperatorContent.maxTabsReached'))
        return
      }
    ...
    const onRightClickOperation = useMemoizedFn((event: React.MouseEvent, item: MultipleNodeInfo) => {
      ...
      if (subIndex !== -1) {
        menuData.splice(2, 0, {
          label: t('MainOperatorContent.removeFromGroup'),
          key: 'removeFromGroup',
        })
      }
      if (currentTabKey === YakitRoute.HTTPFuzzer) {
    ...
    const onRename = useMemoizedFn((val: string, item: MultipleNodeInfo) => {
      if (val.length > 50) {
        yakitNotify('error', t('MainOperatorContent.max50Chars'))
        return
      }
    ...
    const onEditGroupSave = useMemoizedFn((group: MultipleNodeInfo, list: MultipleNodeInfo[], callback: () => void) => {
              ...
              renderClone={(provided, snapshot, rubric) => {
                ...
                return (
                  <div
                    ...
                    style={{ ...cloneStyle }}
                  >
                    <DroppableClone subPage={subPage} selectSubMenu={selectSubMenu} draggableId={rubric.draggableId} />
                  </div>
                )
              }}
            >
...
/**验证组名是否超过50个字符 */
const onVerifyGroupName = (val: string, t: TFunction) => {
  if (val.length > 50) {
    yakitNotify('error', t('MainOperatorContent.max50Chars'))
    return false
  }
...
const GroupRightClickShowContent: React.FC<GroupRightClickShowContentProps> = React.memo((props) => {
  ...
  const menu = useCreation(() => {
    return [
      {
        label: t('MainOperatorContent.cancelGroup'),
        key: 'cancelGroup',
      },
      {
        label: t('MainOperatorContent.closeGroup'),
        key: 'closeGroup',
      },
      {
        label: t('MainOperatorContent.closeOtherTabs'),
      ...
      {
        label: t('MainOperatorContent.editGroup'),
        key: 'editGroup',
      },
    ]
  }, [i18n.language])
      ...
      <div className={styles['show-content-heard']}>
        <YakitInput
          ...
          onChange={(e) => {
            ...
            if (onVerifyGroupName(value, t)) {
              setName(value)
            }
          }}
          onPressEnter={() => {
            if (onVerifyGroupName(name, t)) {
