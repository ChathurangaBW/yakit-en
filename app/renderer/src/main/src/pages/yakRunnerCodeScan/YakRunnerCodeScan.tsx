      ...
      .then(async ({ Group }) => {
        const nextGroup = await Promise.all(
          Group.map(async (item) => ({
            ...item,
            Count: await getGroupNamesTotal({ GroupNames: [item.GroupName], Purpose: pageInfo.Purpose }),
          })),
        )
  ...
  const onSelectAll = useMemoizedFn(async (e) => {
    try {
      ...
      if (checked) {
        ...
        const selectTotal = await getGroupNamesTotal({ GroupNames, Purpose: pageInfo.Purpose })
        setPageInfo((prev) => ({
          ...prev,
          GroupNames,
          Keyword: '',
  ...
  const onSelect = useMemoizedFn(async (val: SyntaxFlowGroup) => {
    try {
      ...
      if (isExist) {
        ...
        setLoading(true)
        const selectTotal = await getGroupNamesTotal({ GroupNames: newList, Purpose: pageInfo.Purpose })
        setLoading(false)
...
const CodeScanRuleSetting: React.FC<CodeScanRuleSettingProps> = React.memo((props) => {
  ...
  const isActive = useMemo(() => {
    return pageInfo?.Purpose?.includes('vuln') || filterLibRuleKind !== 'noLib'
  }, [pageInfo, filterLibRuleKind])
  return (
    <>
      <YakitPopover
        content={
          <div className={styles['code-scan-rule-setting-popover']}>
            <YakitCheckbox
              checked={pageInfo?.Purpose?.includes('vuln')}
              onChange={(e) => onComplianceModeChange(e.target.checked ? 'exclude' : 'include')}
            >
        ...
        <div className={styles['filter-wrapper']}>
          ...
          <div className={styles['filter-body']} style={{ padding: '0px 8px 0px 7px' }}>
            <div className={styles['filter-body-left']}>
              ...
              <span className={styles['count-num']}>
                Selected
                <span className={styles['num-style']}>{checked ? response.Total : selectedRules.length}</span>
              </span>
            </div>
            <div className={styles['filter-body-right']}>
              <CodeScanRuleSetting
                filterLibRuleKind={filterLibRuleKind}
                onFilterLibRuleKindChange={onFilterLibRuleKindChange}
                pageInfo={pageInfo}
...
const CodeScanByGroup: React.FC<CodeScanByGroupProps> = React.memo((props) => {
  const { hidden, filterLibRuleKind, allCheck, setAllCheck, selectGroup, keywords, isRefresh, Purpose } = props
  ...
  useEffect(() => {
    fetchList(true)
  }, [filterLibRuleKind, selectGroup, keywords, isRefresh, Purpose])
  ...
  forwardRef((props, ref) => {
    ...
    const customParams = useMemo(() => {
      ...
      try {
        ...
        const peepholeArr: FormExtraSettingProps = JSONParseLog(peephole, {
          page: 'yakRunnerCodeScan',
          fun: 'peepholeArr',
        }) || {
          double: false,
        ...
        const languageArr: FormExtraSettingProps = JSONParseLog(language, {
          page: 'yakRunnerCodeScan',
          fun: 'languageArr',
        }) || {
          double: false,
    ...
    const onStreamInfoFun = useMemoizedFn(async (newStreamInfo: HoldGRPCStreamInfo) => {
      ...
      if (!isRealStartRef.current) {
        ...
        if (startLog && startLog.data) {
          try {
            const verifyStart = JSONParseLog(startLog?.data, {
              page: 'yakRunnerCodeScan',
              fun: 'onStreamInfoFun',
            }) as VerifyStartProps
    ...
    <div className={styles['flow-rule-item-wrapper']}>
      <div className={styles['flow-rule-item']}>
        ...
        <div className={styles['flow-rule-item-info']}>
          <div className={classNames(styles['flow-rule-item-info-text-style'], 'yakit-content-single-ellipsis')}>
            {data.RuleName}
          </div>
        </div>
