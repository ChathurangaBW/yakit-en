...
const getForbiddenStartupDebugFlags = () => {
  ...
  BLOCKED_CHROMIUM_DEBUG_SWITCHES.forEach((theSwitch) => {
    if (!app.commandLine.hasSwitch(theSwitch)) {
      return
    }

    const value = app.commandLine.getSwitchValue(theSwitch)
    detected.push(value ? `--${theSwitch}=${value}` : `--${theSwitch}`)
  })
  ;[...process.execArgv, ...process.argv].forEach((arg) => {
    if (typeof arg !== 'string') {
      return
    }

    if (
      BLOCKED_NODE_DEBUG_ARG_PREFIXES.some((prefix) => {
...
const removeForbiddenChromiumDebugSwitches = () => {
  BLOCKED_CHROMIUM_DEBUG_SWITCHES.forEach((theSwitch) => {
    try {
      app.commandLine.removeSwitch(theSwitch)
    } catch (error) {}
  })
}

const forbiddenStartupDebugFlags = app.isPackaged ? getForbiddenStartupDebugFlags() : []
const shouldAbortStartupForDebugFlags = app.isPackaged && forbiddenStartupDebugFlags.length > 0

if (shouldAbortStartupForDebugFlags) {
  removeForbiddenChromiumDebugSwitches()
  const blockedMessage =
    `[Main] blocked packaged startup due to forbidden debug flags: ` + `${forbiddenStartupDebugFlags.join(', ')}`

  console.error(blockedMessage)
  try {
    printLogOutputFile(blockedMessage)
  } catch (error) {}
